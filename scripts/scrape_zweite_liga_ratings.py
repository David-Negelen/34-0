#!/usr/bin/env python3
"""
Scrape player OVR ratings from fifaindex.com into zweite_liga_draft.db.

Works identically to scrape_ratings.py but targets the 2. Bundesliga DB
and reads club IDs from data/fifaindex_zweite_liga_clubs.txt.

Usage:
    python scripts/scrape_zweite_liga_ratings.py [--debug]
...
CLUBS_FILE   = ROOT / "data" / "fifaindex_zweite_liga_clubs.txt"
...
def main():
    ...
    if not DB_PATH.exists():
        sys.exit(f"DB not found: {DB_PATH}\nRun scrape_zweite_liga.py first.")
    if not CLUBS_FILE.exists():
        sys.exit(
            f"Clubs file not found: {CLUBS_FILE}\n"
            "Add entries like:  123-hamburger-sv"
        )
